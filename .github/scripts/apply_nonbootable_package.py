...
if text.count(old) != 1:
    raise SystemExit("man page loop marker is not unique")
...
if text.count(old) != 1:
    raise SystemExit("dependency marker is not unique")
text = text.replace(old, new, 1)
if text.count("rufusarm64-nonbootable-format") != 5:
    raise SystemExit("unexpected formatter reference count in build-deb.sh")
...
"""
if text.count("</policyconfig>") != 1:
    raise SystemExit("policy closing marker is not unique")
