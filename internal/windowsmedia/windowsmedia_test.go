...
func TestCreateWithFakeSystemTools(t *testing.T) {
	...
	err := Create(context.Background(), target, target, Options{
...
func installFakeTools(t *testing.T, directory string) {
	...
	tools := []string{"wipefs", "udevadm", "mkfs.vfat", "fsck.vfat", "sync"}
...
func TestInspectMountedISORejectsFATCaseCollision(t *testing.T) {
	...
	if _, err := inspectMountedISO(root); err == nil || !strings.Contains(err.Error(), "collide") {
...
func TestCreateUsesCustomLabelAndDoesNotWaitForGlobalUdevQueue(t *testing.T) {
	...
	target := filepath.Join(t.TempDir(), "fake-device")
	writeTestFile(t, target, make([]byte, 1024))
	if err := Create(context.Background(), target, target, Options{TargetSize: 8 * 1024 * 1024 * 1024, VolumeLabel: "WIN11"}, nil); err != nil {
		t.Fatal(err)
	}
	data, err := os.ReadFile(logPath)
	if err != nil {
	...
	logText := string(data)
	if !strings.Contains(logText, "mkfs.vfat  -F 32 -n WIN11") && !strings.Contains(logText, "mkfs.vfat -F 32 -n WIN11") {
		t.Fatalf("custom label missing from mkfs command:\n%s", logText)
	}
