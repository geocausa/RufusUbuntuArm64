...
func ValidateTarget(path string, dev device.BlockDevice, allowFixed bool) error {
	...
	if !allowFixed && !dev.Removable && dev.Transport != "usb" && dev.Transport != "mmc" {
		return fmt.Errorf("target %s is not marked removable or USB/MMC; use --allow-fixed only after checking the device carefully", path)
	}
...
func EnsureImageNotOnTarget(imagePath, targetPath string) error {
	...
	if err != nil {
		...
		return fmt.Errorf("cannot identify the image file's backing disk: %w", err)
	}
	if contains(imageDisks, filepath.Base(targetPath)) {
		return fmt.Errorf("image file is stored on the target disk %s; move the image to another disk before writing", targetPath)
	}
	return nil
}
...
func UnmountDescendants(dev device.BlockDevice) error {
	...
	for i := len(mounted) - 1; i >= 0; i-- {
		node := mounted[i]
		for j := len(node.Mountpoints) - 1; j >= 0; j-- {
			mountpoint := node.Mountpoints[j]
			cmd := exec.Command("umount", "--", mountpoint)
			var stderr bytes.Buffer
			cmd.Stderr = &stderr
			if err := cmd.Run(); err != nil {
				return fmt.Errorf("unmount %s (%s): %w: %s", node.Path, mountpoint, err, strings.TrimSpace(stderr.String()))
			}
		}
	}
	return nil
...
func RereadPartitionTable(path string) error {
	cmd := exec.Command("blockdev", "--rereadpt", path)
	var stderr bytes.Buffer
	cmd.Stderr = &stderr
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("blockdev --rereadpt: %w: %s", err, strings.TrimSpace(stderr.String()))
	}
	return nil
}
