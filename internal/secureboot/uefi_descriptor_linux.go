...
func openUEFIMediaTree(ctx context.Context, root string, maxFiles int, hook uefiTraversalHook) (string, []uefiMediaFile, []string, error) {
	...
	var warnings []string
	if err := walkUEFIDirectory(ctx, rootFile, "", maxFiles, hook, &files, &warnings); err != nil {
		return "", nil, nil, err
	}
...
func walkUEFIDirectory(ctx context.Context, directory *os.File, prefix string, maxFiles int, hook uefiTraversalHook, files *[]uefiMediaFile, warnings *[]string) error {
	if err := ctx.Err(); err != nil {
		return err
	}
	entries, err := directory.ReadDir(-1)
	if err != nil {
		return fmt.Errorf("read UEFI directory %q: %w", prefix, err)
	}
	...
	for _, entry := range entries {
		...
		if entryInfo.IsDir() {
			...
			err = walkUEFIDirectory(ctx, child, relative, maxFiles, hook, files, warnings)
		...
		if len(*files) >= maxFiles {
			return fmt.Errorf("more than %d EFI executables found; refusing an unbounded scan", maxFiles)
		}
		if hook != nil {
			hook("entry-before-open", relative)
		}
