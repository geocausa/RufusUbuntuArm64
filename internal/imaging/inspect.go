...
func InspectOpenFile(file *os.File) (ImageInfo, error) {
	...
	return InspectReaderAt(file, stat.Size())
}

// InspectReaderAt performs the same bounded inspection on any random-access
// byte source. size is the logical image size, not necessarily the number of
// bytes currently materialized by the reader.
func InspectReaderAt(reader io.ReaderAt, size int64) (ImageInfo, error) {
...
func inspectGPT(file io.ReaderAt, sector []byte, fileSectors uint64) bool {
...
func inspectOpticalDescriptors(file io.ReaderAt, info *ImageInfo) {
