...
- A guarded **Restore / format…** workflow for verified data-only GPT/MBR media using FAT16, FAT32, exFAT, NTFS, or ext4.
...
Select a removable target and choose **Restore / format…**. RufusArm64 calculates an unprivileged identity-bound plan for GPT or MBR and FAT16, FAT32, exFAT, NTFS, or ext4. It displays geometry, required tools, warnings, and an exact `FORMAT` phrase before administrator authentication.

FAT16 is admitted only when the canonical whole-drive partition remains below 4 GiB; larger targets fail before erasure rather than being silently truncated. A successful report verifies the partition geometry and filesystem. It does not claim bootability. Cancellation before erasure leaves the drive unchanged; cancellation or failure after erasure reports changed incomplete media conservatively.
